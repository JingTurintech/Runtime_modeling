...
import itertools
from utils import NaiveModel_mean, NaiveModel_median, get_MAPE, get_RMSE, interpretate_cohen_d
...
def main():
    ...
    selected_models = ['Naive_mean', 'BayesianRR']
    dataset_folder_path = '{}/Runtime_modeling/optimization_data'.format(os.curdir)
    saving_folder_path = '{}/Runtime_modeling/results'.format(os.curdir)
    all_files = sorted(os.listdir(dataset_folder_path)) # Get all files in the directory and sort them
            ...
            for current_performance in seletected_performance_metrics:
                ...
                for run in range(1, total_runs + 1):
                    ...
                    for current_model in selected_models:
                        ...
                        elif current_model == 'Naive_mean':
                            model = NaiveModel_mean()

                        elif current_model == 'Naive_median':
                            model = NaiveModel_median()
    ...
    if save_results:
        ...
        results_df.to_csv("{}/{}_vs_{}_results.csv".format(saving_folder_path, selected_models[0], selected_models[1]), index=False)
        print('Results saved to {}'.format("{}/{}_vs_{}_results.csv".format(saving_folder_path, selected_models[0], selected_models[1])))
